...
<head>
    <meta charset="utf-8">
    <title>Stockmaster - 월별 재고관리</title>
...
<body>
    <div class="container-fluid position-relative bg-white d-flex p-0">
        ...
        <div class="content">
            ...
            <!-- Monthly Inventory Management Start -->
            <div class="container-fluid pt-4 px-4">
                <div class="row g-4">
                    <div class="col-12">
                        <div class="bg-light rounded h-100 p-4">
                            <h6 class="mb-4">월별 재고조회</h6>
                            <!-- Search Filters -->
                            <form class="row g-3 mb-4 align-items-end" action="${pageContext.request.contextPath}/inventory/monthly/search.do" method="post">
                                <div class="col-md-3">
                                    <label for="period" class="form-label">기간</label>
                                    <div class="input-group">
                                        <input type="month" class="form-control" name="startMonth" value="${param.startMonth}">
                                        <span class="input-group-text">~</span>
                                        <input type="month" class="form-control" name="endMonth" value="${param.endMonth}">
                                    </div>
                                </div>
                                <div class="col-md-3">
                                ...
                                <table class="table text-center align-middle table-bordered table-hover mb-0">
                                    <thead>
                                        <tr class="text-dark">
                                            ...
                                            <c:forEach items="${monthHeaders}" var="month">
                                                <th scope="col">${month}</th>
                                            </c:forEach>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <c:if test="${not empty monthlyInventoryList}">
                                            <c:forEach items="${monthlyInventoryList}" var="item">
                                                <tr>
                                                    <td>${item.supplier}</td>
                                                    ...
                                                    <c:forEach items="${item.monthlyQuantities}" var="quantity">
                                                        <td>${quantity}</td>
                                                    </c:forEach>
                                                </tr>
                                            </c:forEach>
                                        </c:if>
                                        <c:if test="${empty monthlyInventoryList}">
                                            <tr>
                                                <td colspan="${8 + fn:length(monthHeaders)}" class="text-center">데이터가 없습니다.</td>
                                            </tr>
                                        </c:if>
                                    </tbody>
                                </table>
                                ...
                                <nav aria-label="Page navigation" class="mt-4 d-flex justify-content-center">
                                    <ul class="pagination">
                                        <li class="page-item ${pageInfo.currentPage eq 1 ? 'disabled' : ''}">
                                            <a class="page-link" href="${pageContext.request.contextPath}/inventory/monthly.do?page=${pageInfo.currentPage - 1}" aria-label="Previous">
                                                <span aria-hidden="true">&laquo;</span>
                                            </a>
                                        </li>
                                        <c:forEach begin="1" end="${pageInfo.totalPages}" var="pageNum">
                                            <li class="page-item ${pageInfo.currentPage eq pageNum ? 'active' : ''}">
                                                <a class="page-link" href="${pageContext.request.contextPath}/inventory/monthly.do?page=${pageNum}">${pageNum}</a>
                                            </li>
                                        </c:forEach>
                                        <li class="page-item ${pageInfo.currentPage eq pageInfo.totalPages ? 'disabled' : ''}">
                                            <a class="page-link" href="${pageContext.request.contextPath}/inventory/monthly.do?page=${pageInfo.currentPage + 1}" aria-label="Next">
            ...
            <!-- Monthly Inventory Management End -->
        </div>
        <!-- Content End -->
